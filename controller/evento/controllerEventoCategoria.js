    ...
    try {
        if (String(contentType).toLowerCase() == 'application/json') {
            if (id == '' || id == undefined || id == null || isNaN(id) || id <= 0 ||
                ...
                eventoCategoria.id_categoria == '' || eventoCategoria.id_categoria == undefined || eventoCategoria.id_genero == null || isNaN(eventoCategoria.id_genero) || eventoCategoria.id_categoria <= 0
            ) {
                return message.ERROR_REQUIRED_FIELDS //400
            } else {
                //Validação para verificar se o ID existe no BD
                let resultCategoria = await eventoCategoriaDAO.selectByIdFilmeGenero(parseInt(id))
